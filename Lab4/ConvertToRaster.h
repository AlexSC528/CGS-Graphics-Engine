...
void ClearWindow(unsigned int _newColor, unsigned int* _raster)
{
...
void NDCToScreen(VertexStruct& cartesian)
{
	...
	cartesian = { static_cast<float>(screenX), static_cast<float>(screenY), 0, 0, cartesian.color };
}
	...
	for (int y = StartY; y < EndY; y++)
	{
		for (int x = StartX; x < EndX; x++)
		{
			...
			if (bya.X >= 0 && bya.X <= 1 && bya.Y >= 0 && bya.Y <= 1 && bya.Z >= 0 && bya.Z <= 1)
			{
				...
				VertexStruct pointToDraw{ static_cast<float>(x),  static_cast<float>(y), 0, 0 };
